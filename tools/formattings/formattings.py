...
def set_text_style(sheet_name:str, cell_range:str, bold:Optional[bool]=None, italic:Optional[bool]=None, font_size:Optional[int]=None)->str:
    """Applies bold, italic, and font size styles to a range of cells."""
    ...
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Text style applied"
...
def set_text_color(sheet_name:str, cell_range:str, r:int, g:int, b:int)->str:
    """Sets the foreground (text) color for a range of cells using RGB."""
    fmt={Constants.TEXT_FORMAT:{Constants.FOREGROUND_COLOR:utility.normalize_color((r,g,b))}}
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Text color applied"
...
def set_background_color(sheet_name:str, cell_range:str, r:int, g:int, b:int)->str:
    """Sets the background (fill) color for a range of cells using RGB."""
    fmt={Constants.BACKGROUND_COLOR:utility.normalize_color((r,g,b))}
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Background color applied"
...
def set_alignment(sheet_name:str, cell_range:str, h_align:Optional[str]=None, v_align:Optional[str]=None)->str:
    """Sets horizontal and vertical text alignment within a range of cells."""
    fmt={}
    if h_align: fmt[Constants.H_ALIGN]=h_align.upper()
    if v_align: fmt[Constants.V_ALIGN]=v_align.upper()
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Alignment applied"
...
def set_wrap(sheet_name:str, cell_range:str, wrap:str)->str:
    """Sets the text wrapping strategy (WRAP, CLIP, OVERFLOW) for a range."""
    fmt={Constants.WRAP:wrap.upper()}
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Wrap applied"
...
def set_borders(sheet_name:str, cell_range:str, style:str="SOLID")->str:
    """Applies cell borders with a specified style to a range of cells."""
    ...
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Borders applied"
...
def set_rotation(sheet_name:str, cell_range:str, angle:int)->str:
    """Rotates the text within a range of cells by a specific angle."""
    fmt={Constants.TEXT_ROTATION:{Constants.ANGLE:angle}}
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Rotation applied"
...
def set_number_format(sheet_name:str, cell_range:str, format_string:str)->str:
    """Sets the number or date format pattern for a range of cells."""
    ...
    mask = utility.get_field_mask(fmt)
    utility._get_worksheet(sheet_name).apply_format(cell_range, fmt,fields=mask); return "Number format applied"
...
def merge_cells(sheet_name:str, start_range:str,end_range:str)->str:
    """Merges a specified rectangular range of cells into a single cell."""
...
def set_column_width(sheet_name:str, column:int, width:Optional[int]=None)->str:
    """Sets the width of a specific column in the worksheet."""
...
def set_row_height(sheet_name:str, row:int, height:Optional[int]=None)->str:
    """Sets the height of a specific row in the worksheet."""
...
def auto_fit_columns(sheet_name:str, start_column:int, end_column:int)->str:
    """Automatically fits column widths to their content for a specified range."""
